...
def main():
    ...
    for k, m in enumerate(momentum):
        ...
        train_loss, train_err, val_err = run(
            x_train, y_train, x_test, y_test, nn, epochs
        )
        print(
            "    final: train error: %0.5f, val error: %0.5f"
        ...
        np.save(
            "mnist_nn_experiments_momentum/train_error_30k_%0.2f.npy" % m, train_err
        )
        np.save(
            "mnist_nn_experiments_momentum/train_loss_30k_%0.2f.npy" % m, train_loss
        )
